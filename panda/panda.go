...
const (
	generatedHexSecretPrefix = "r!"
	// generatedHexSecretPrefix2 is used to indicate that scrypt should
	...
	// them.
	generatedHexSecretPrefix2  = "r["
	generatedWordsSecretPrefix = "w!"
)
...
func NewSecretString(rand io.Reader) string {
	...
	// Imho 10 words seems excessive but dicewords only has 7776 words.
	// So 10 words gives 129 bits, agreeing with our 16 bytes above,
	// while 8 words gives 103 bits, and 6 only 77 bits.
	// We should find a larger dictionary than dicewrods probably.
	s := diceware.DicewareWords(rand, " ", 10)
	...
	return generatedWordsSecretPrefix + hex.EncodeToString(digest[0:2]) + s
}

func hasGeneratedHexPrefix(s string) bool {
	return strings.HasPrefix(s, generatedHexSecretPrefix) ||
		strings.HasPrefix(s, generatedHexSecretPrefix2)
}

func hasGeneratedWordsPrefix(s string) bool {
	return strings.HasPrefix(s, generatedWordsSecretPrefix)
}

func hasGeneratedPrefix(s string) bool {
	return hasGeneratedWordsPrefix(s) || hasGeneratedHexPrefix(s)
}

// isValidSecretString returns true if s is of the form generated by
// NewSecretString.
func isValidSecretString(s string) bool {
	if hasGeneratedWordsPrefix(s) {
		s = s[len(generatedWordsSecretPrefix):]
		var digest0 [2]byte
		if _, err := hex.Decode(digest0[:], []byte(s[0:4])); err != nil {
			return false
		}
		...
		return digest0[0] == digest1[0] && digest0[1] == digest1[1]
	}
	if hasGeneratedHexPrefix(s) {
		s = s[len(generatedHexSecretPrefix):]
...
// generatedHexSecretPrefix but don't have a matching checksum.
func IsAcceptableSecretString(s string) bool {
	// Should test length if we've hasGeneratedWordsPrefix?
	return !hasGeneratedPrefix(s) || isValidSecretString(s)
}
...
func (s *SharedSecret) isStrongRandom() bool {
	return strings.HasPrefix(s.Secret, generatedHexSecretPrefix2) && isValidSecretString(s.Secret)
}
